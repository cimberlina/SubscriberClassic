...
FunctionalState GPIO_GetIntStatus(uint8_t portNum, uint32_t pinNum, uint8_t edgeState)
{
	if((portNum == 0) && (edgeState == 0))//Rising Edge
		return ((((LPC_GPIOINT->IO0IntStatR)>>pinNum)& 0x1)?ENABLE:DISABLE);
	else if ((portNum == 2) && (edgeState == 0))
		return ((((LPC_GPIOINT->IO2IntStatR)>>pinNum)& 0x1)?ENABLE:DISABLE);
	else if ((portNum == 0) && (edgeState == 1))//Falling Edge
		return ((((LPC_GPIOINT->IO0IntStatF)>>pinNum)& 0x1)?ENABLE:DISABLE);
	else if ((portNum == 2) && (edgeState == 1))
		return ((((LPC_GPIOINT->IO2IntStatF)>>pinNum)& 0x1)?ENABLE:DISABLE);
	else
		//Error
		while(1);
}
