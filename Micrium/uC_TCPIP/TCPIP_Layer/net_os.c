...
static	OS_TCB		NetOS_Tmr_TaskTCB;
static	OS_TCB		NetOS_IF_RxTaskTCB;

/* -------------------- TASK STKs --------------------- */
static  CPU_STK     NetOS_Tmr_TaskStk[NET_OS_CFG_TMR_TASK_STK_SIZE];
static  CPU_STK     NetOS_IF_RxTaskStk[NET_OS_CFG_IF_RX_TASK_STK_SIZE];


                                                                /* --------------------- SIGNALS ---------------------- */
static  OS_SEM  NetOS_InitSignalPtr;
static  OS_SEM  NetOS_NIC_TxRdySignalPtr;

#if    (NET_CFG_LOAD_BAL_EN == DEF_ENABLED)
static  OS_SEM  NetOS_TxSuspendSignalPtr;
#endif

#ifdef  NET_SOCK_MODULE_PRESENT
#if    (NET_SOCK_CFG_TYPE_STREAM_EN == DEF_ENABLED)
static  OS_SEM  NetOS_Sock_ConnReqSignalPtr[NET_SOCK_CFG_NBR_SOCK];
static  OS_SEM  NetOS_Sock_ConnAcceptQ_SignalPtr[NET_SOCK_CFG_NBR_SOCK];
static  OS_SEM  NetOS_Sock_ConnCloseSignalPtr[NET_SOCK_CFG_NBR_SOCK];
#endif
#endif

                                                                /* ---------------------- LOCKS ----------------------- */
static  OS_SEM  NetOS_LockPtr;


                                                                /* --------------------- QUEUES ----------------------- */
static  OS_SEM  NetOS_IF_RxQ_SignalPtr;


                                                                /* ---------------------- SEMS ------------------------ */
#ifdef  NET_TCP_MODULE_PRESENT
static  OS_SEM  NetOS_TCP_RxQ_SignalPtr[NET_TCP_CFG_NBR_CONN];
static  OS_SEM  NetOS_TCP_TxQ_SignalPtr[NET_TCP_CFG_NBR_CONN];
#endif

#ifdef  NET_SOCK_MODULE_PRESENT
static  OS_SEM  NetOS_Sock_RxQ_SignalPtr[NET_SOCK_CFG_NBR_SOCK];
#endif
...
#if    (NET_CFG_LOAD_BAL_EN == DEF_ENABLED)
static  CPU_INT16U     NetOS_TxSuspendTimeout_tick;
#endif

#ifdef  NET_TCP_MODULE_PRESENT
static  CPU_INT16U     NetOS_TCP_RxQ_TimeoutTbl_tick[NET_TCP_CFG_NBR_CONN];
static  CPU_INT16U     NetOS_TCP_TxQ_TimeoutTbl_tick[NET_TCP_CFG_NBR_CONN];
#endif

#ifdef  NET_SOCK_MODULE_PRESENT
static  CPU_INT16U     NetOS_Sock_RxQ_TimeoutTbl_tick[NET_SOCK_CFG_NBR_SOCK];

#if    (NET_SOCK_CFG_TYPE_STREAM_EN == DEF_ENABLED)
static  CPU_INT16U     NetOS_Sock_ConnReqTimeoutTbl_tick[NET_SOCK_CFG_NBR_SOCK];
static  CPU_INT16U     NetOS_Sock_ConnAcceptQ_TimeoutTbl_tick[NET_SOCK_CFG_NBR_SOCK];
static  CPU_INT16U     NetOS_Sock_ConnCloseTimeoutTbl_tick[NET_SOCK_CFG_NBR_SOCK];
#endif
#endif
...
static  CPU_INT16U      NetOS_TimeoutCalc_OS_tick(CPU_INT32U    timeout_ms,
                                              CPU_BOOLEAN   lim_max,
                                              NET_ERR      *perr);

static  CPU_INT32U  NetOS_TimeoutCalc_ms     (CPU_INT16U        timeout_os_tick);
...
void  NetOS_Init (NET_ERR  *perr)
{
    ...
    /* Create network initialization signal (see Note #1a).     */
    OSSemCreate(&NetOS_InitSignalPtr,
    			NET_INIT_NAME,
    			0,
    			&err);
    ...
    /* Create network lock (see Note #1b).                      */
    OSSemCreate(&NetOS_LockPtr,
				NET_LOCK_NAME,
				1,
				&err);
...
#if (NET_CFG_LOAD_BAL_EN == DEF_ENABLED)
	/* Create network transmit suspend signal (see Note #1c).   */
	OSSemCreate(&NetOS_TxSuspendSignalPtr,
				NET_TX_SUSPEND_NAME,
				0,
				&err);
...
void  NetOS_InitWait (NET_ERR  *perr)
{
    ...
    OSSemPend(&NetOS_InitSignalPtr,                 /* Wait until network initialization completes ...      */
              0,                                   /* ... without timeout (see Note #1).                   */
...
void  NetOS_InitSignal (NET_ERR  *perr)
{
    ...
    OSSemPost(&NetOS_InitSignalPtr,
    		  OS_OPT_POST_ALL,
    		  &os_err);                    /* Signal network initialization complete.              */
...
void  NetOS_Lock (NET_ERR  *perr)
{
    ...
    OSSemPend(&NetOS_LockPtr,                 /* Wait until network initialization completes ...      */
              0,                                   /* ... without timeout (see Note #1).                   */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
...
void  NetOS_TxSuspendWait (void)
{
    OS_ERR  os_err;
     ...
     OSSemPend(&NetOS_LockPtr,
    		  NetOS_TxSuspendTimeout_tick,
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
...
void  NetOS_TxSuspendSignal (void)
{
   ...
   /* Signal network transmit suspend.                     */
   OSSemPost(&NetOS_TxSuspendSignalPtr,
       		  OS_OPT_POST_1,
       		  &os_err);
}
#endif
...
{
    OS_ERR  os_err;
    unsigned int dbgerror;

                                                                            /* Create NetOS_Tmr_Task() [see Note #1a].  */
    OSTaskCreate(&NetOS_Tmr_TaskTCB,
    			 NET_TMR_TASK_NAME,
    			 NetOS_Tmr_Task,
				 (void *)0,
...
static  void  NetOS_Tmr_Task (void  *p_data)
{
   ...
   (void)&p_data;                                               /* Prevent compiler warning.                            */
    ...
    dly = OS_CFG_TICK_RATE_HZ / NET_TMR_CFG_TASK_FREQ;             /* Delay task at NET_TMR_CFG_TASK_FREQ rate.            */
...
void  NetOS_NIC_Init (NET_ERR  *perr)
{
    ...
    OSSemCreate(&NetOS_NIC_TxRdySignalPtr,
			NET_NIC_TX_RDY_NAME,
...
void  NetOS_NIC_TxRdyWait (NET_ERR  *perr)
{
    ...
    /* Wait on NIC transmit ready signal ...                */
    OSSemPend(&NetOS_NIC_TxRdySignalPtr,
    		  0,									/* ... preferably without timeout (see Note #1a).       */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
...
void  NetOS_NIC_TxRdySignal (void)
{
    ...
    OSSemPost(&NetOS_NIC_TxRdySignalPtr,
              OS_OPT_POST_1,
              &os_err);
...
void  NetOS_IF_Init (NET_ERR  *perr)
{
    ...
    OSSemCreate(&NetOS_IF_RxQ_SignalPtr,
    			NET_IF_RX_Q_NAME,
    			0,
    			&os_err);
    ...
    OSTaskCreate(&NetOS_IF_RxTaskTCB,
    			 NET_IF_RX_TASK_NAME,
    			 NetOS_IF_RxTask,
    			 (void *)0,
...
//              (CPU_INT16U    ) 0,                                   /* ... preferably without timeout (see Note #1a).       */
//              (CPU_INT08U    *)&os_err);

    OSSemPend(&NetOS_IF_RxQ_SignalPtr,
    		  0,									/* ... preferably without timeout (see Note #1a).       */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
...
void  NetOS_IF_RxTaskSignal (NET_ERR  *perr)
{
    ...
    OSSemPost(&NetOS_IF_RxQ_SignalPtr,
              OS_OPT_POST_1,
              &os_err);                 /* Signal network interface receive queue.              */
                                          ...
                                          NET_ERR      *perr)
{
    ...
    CPU_CRITICAL_ENTER();
    id     = (CPU_INT16U)OSPrioCur;                   /* Set task prio as ICMP Req Msg id (see Note #3b).     */
    CPU_CRITICAL_EXIT();
...
void  NetOS_TCP_Init (NET_ERR  *perr)
{
    ...
    for (i = 0; i < NET_TCP_CFG_NBR_CONN; i++) {
        OSSemCreate(psignal,								/* Create     TCP connection receive queue semaphores.      */
			"",
			0,
			&err);
                          ...
                          NET_ERR          *perr)
{
    ...
    OSSemPend(psignal,
    		  timeout_os_tick,									/* ... preferably without timeout (see Note #1a).       */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
                           ...
                           NET_ERR          *perr)
{
    ...
    psignal = &(NetOS_TCP_RxQ_SignalPtr[conn_id_tcp]);
   (void)OSSemPendAbort(psignal,                   /* Abort wait on TCP connection receive queue ...       */
                        OS_OPT_PEND_ABORT_ALL,     /* ... for ALL waiting tasks.                           */
                        &os_err);
                          ...
                          NET_ERR          *perr)
{
    ...
    OSSemPend(psignal,
    		  timeout_os_tick,									/* ... with configured timeout (see Note #1b).          */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
                            ...
                            NET_ERR          *perr)
{
    ...
    ctr  = OSSemPost(psignal,
    				 OS_OPT_POST_1,
    				 &os_err);                               /* Signal TCP connection transmit queue.                */
                           ...
                           NET_ERR          *perr)
{
    ...
    psignal = &(NetOS_TCP_TxQ_SignalPtr[conn_id_tcp]);
    OSSemPendAbort(psignal,                   /* Abort wait on TCP connection transmit queue ...      */
                   OS_OPT_PEND_ABORT_ALL,     /* ... for ALL waiting tasks.                           */
                   &os_err);
...
void  NetOS_Sock_Init (NET_ERR  *perr)
{
    ...
    for (i = 0; i < NET_SOCK_CFG_NBR_SOCK; i++) {
       OSSemCreate(psignal,								/* Create     socket connection signal semaphores.      */
				"",
				0,
				&err);
      if( err != OS_ERR_NONE )	{
                           ...
                           NET_ERR      *perr)
{
    ...
    OSSemPend(psignal,											/* Wait on socket receive queue ...                     */
        	  timeout_os_tick,									/* ... with configured timeout (see Note #1b).          */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
                             ...
                             NET_ERR      *perr)
{
    ...
    ctr  = OSSemPost(psignal,
    				 OS_OPT_POST_1,
    				 &os_err);                               /* Signal socket receive queue.                         */
                            ...
                            NET_ERR      *perr)
{
    ...
    psignal = &(NetOS_Sock_RxQ_SignalPtr[sock_id]);
    OSSemPendAbort(psignal,                   /* Abort wait on socket receive queue ...               */
                   OS_OPT_PEND_ABORT_ALL,     /* ... for ALL waiting tasks.                           */
                   &os_err);
                              ...
                              NET_ERR      *perr)
{
    ...
    OSSemPend(psignal,											/* Wait on socket connection request signal ...         */
              timeout_os_tick,									/* ... with configured timeout (see Note #1b).          */
              OS_OPT_PEND_BLOCKING,
              0,
              &os_err);
                                ...
                                NET_ERR      *perr)
{
    ...
    ctr  = OSSemPost(psignal,
    					OS_OPT_POST_1,
    					&os_err);                               /* Signal socket connection.                            */
                               ...
                               NET_ERR      *perr)
{
    ...
    psignal = &(NetOS_Sock_ConnReqSignalPtr[sock_id]);
    OSSemPendAbort(psignal,                   /* Abort wait on socket connection request signal ...   */
                  OS_OPT_PEND_ABORT_ALL,     /* ... for ALL waiting tasks.                           */
                  &os_err);
                                   ...
                                   NET_ERR      *perr)
{
    ...
    ctr = OSSemPend(psignal,											/* Wait on socket connection accept queue signal ...    */
    				timeout_os_tick,									/* ... with configured timeout (see Note #1b).          */
    				OS_OPT_PEND_BLOCKING,
    				0,
    				&os_err);
                                     ...
                                     NET_ERR      *perr)
{
    ...
    ctr  = OSSemPost(psignal,
    				 OS_OPT_POST_1,
    				 &os_err);                               /* Signal socket accept queue.                          */
                                    ...
                                    NET_ERR      *perr)
{
    ...
    psignal = &(NetOS_Sock_ConnAcceptQ_SignalPtr[sock_id]);
    OSSemPendAbort(psignal,                   /* Abort wait on socket connection accept queue signal  */
                   OS_OPT_PEND_ABORT_ALL,     /* ... for ALL waiting tasks.                           */
                   &os_err);
                                ...
                                NET_ERR      *perr)
{
    ...
    ctr = OSSemPend(psignal,											/* Wait on socket connection close signal ...           */
       				timeout_os_tick,									/* ... with configured timeout (see Note #1b).          */
       				OS_OPT_PEND_BLOCKING,
       				0,
       				&os_err);
                                 ...
                                 NET_ERR      *perr)
{
    ...
    psignal = &(NetOS_Sock_ConnCloseSignalPtr[sock_id]);
    OSSemPendAbort(psignal,                   /* Abort wait on socket connection close signal ...     */
                   OS_OPT_PEND_ABORT_ALL,     /* ... for ALL waiting tasks.                           */
                   &os_err);
                         ...
                         if (pos_event == pos_event_rdy) {              /* .. & if ready OS event found, demultiplex .. */
                             ...
                             if (os_err == OS_ERR_NONE) {               /* If NO OS event-ready error, ...              */
                                 if (pos_event != (OS_EVENT *)0) {
                                    (void)OSSemPost(pos_event,
                                                    OS_OPT_POST_1,
    				                    &os_err); );         /* ... re-signal consumed OS event.             */
                                 }
                             }
                         }
                                           ...
                                           NET_ERR      *perr)
{
    ...
    if (timeout_ms != NET_TMR_TIME_INFINITE) {                  /* If timeout specified, ...                            */
        ...
        timeout_tick = (timeout_ms * OS_CFG_TICK_RATE_HZ) / DEF_TIME_NBR_mS_PER_SEC;
        ...
        if (timeout_tick > 0) {                                 /* If timeout value > 0, ...                            */
            timeout_os_tick = (CPU_INT16U)timeout_tick;             /* ... cast to OS tick size.                            */
        } else {
            timeout_os_tick = (CPU_INT16U)NET_OS_TIMEOUT_MIN_TICK;  /* Else set at least one OS tick (see Note #1a2).       */
        }

    } else {                                                    /* Else for infinite/NO timeout, ...                    */
        timeout_os_tick = (CPU_INT16U)0;                            /* ...  set 0 OS ticks (see Note #1a1).                 */
    }
...
static  CPU_INT32U  NetOS_TimeoutCalc_ms (CPU_INT16U  timeout_os_tick)
{
    ...
    if (timeout_os_tick == 0) {                                 /* If NO OS ticks set, ...                              */
        ...
        timeout_ms = ((CPU_INT32U)timeout_os_tick * DEF_TIME_NBR_mS_PER_SEC) / OS_CFG_TICK_RATE_HZ;
    }
                     ...
                     NET_ERR     *perr)
{
    ...
    time_hr  = (CPU_INT08U ) (time_dly_sec_tot                             / DEF_TIME_NBR_SEC_PER_HR );
    time_min = (CPU_INT08U )((time_dly_sec_tot % DEF_TIME_NBR_SEC_PER_HR ) / DEF_TIME_NBR_SEC_PER_MIN);
    time_sec = (CPU_INT08U ) (time_dly_sec_tot % DEF_TIME_NBR_SEC_PER_MIN);
    time_ms  = (CPU_INT16U) (time_dly_ms      % DEF_TIME_NBR_mS_PER_SEC );

    os_err   = (CPU_INT08U )OSTimeDlyHMSM((CPU_INT08U )time_hr,
                                     (CPU_INT08U )time_min,
                                     (CPU_INT08U )time_sec,
                                     (CPU_INT16U)time_ms);
